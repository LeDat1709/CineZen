...
export default function SeriesPage() {
  ...
  const fetchGenres = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/genres?type=SERIES`)
  ...
  const fetchContents = async () => {
    setLoading(true)
    try {
      let url = `${process.env.NEXT_PUBLIC_API_URL}/contents/series?page=${page}&limit=24`
